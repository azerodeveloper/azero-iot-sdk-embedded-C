...
- [目录](#目录)
- [概述](#概述)
- [端上SDK demo](#端上sdk-demo)

# 概述
Azero IoT Device SDK 是符合C99标准的嵌入式C程序库, 用于安全地接入Azero IoT平台, 仅需简单的设置即可以快速远程控制您的设备. 和AWS IoT 平台设备控制规则完全兼容, 目前支持Shadow服务.
...
static void disconnect_cb(disconnect_reason_t reason)
{
    ...
    if(reason == AZERO_IOT_MQTT_KEEP_ALIVE_TIMEOUT){
...
static void deal_data_cb(const char *key, const char* value,size_t size, bool need_report)
{
    ...
    if(need_report){
        char *format = "{\"%s\":%.*s}";
        size_t length = strlen(format) + size + key;
        char data[length+1];
